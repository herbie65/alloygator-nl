  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingColor) {
        ...
        // Use FirebaseService to create color with logical ID
        await FirebaseService.createProductColor(colorData)
      }
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {(colors as any[]).map((color) => (
                    <tr key={color.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{color.id}</td>
