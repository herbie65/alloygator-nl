  ...
  const handleVatSettingUpdate = async (setting: VatSetting) => {
    ...
    try {
      if (setting.id) {
        ...
        await FirebaseClientService.updateDocument('vat_settings', setting.id, setting);
  ...
  const handleDisplaySettingUpdate = async (settingKey: string, settingValue: boolean) => {
    ...
    try {
      ...
      if (existingSetting && existingSetting.id) {
        // Update existing setting
        await FirebaseClientService.updateDocument('vat_display_settings', existingSetting.id, {
  ...
  const removeVatSetting = async (id: string) => {
    try {
      if (id) {
        ...
        await FirebaseClientService.deleteDocument('vat_settings', id);
