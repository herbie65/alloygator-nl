...
export default function CRMPage() {
  ...
  const [columnConfig, setColumnConfig] = useState({
    company_name: { visible: true, width: 200, order: 0 },
    contact_person: { visible: true, width: 150, order: 1 },
    address: { visible: true, width: 200, order: 2 },
    email: { visible: true, width: 180, order: 3 },
    total_spent: { visible: true, width: 120, order: 4 },
    total_orders: { visible: true, width: 100, order: 5 },
    last_contact: { visible: true, width: 120, order: 6 },
    last_visit: { visible: true, width: 120, order: 7 },
    target: { visible: true, width: 100, order: 8 },
    status: { visible: true, width: 100, order: 9 },
    actions: { visible: false, width: 150, order: 10 }
  })
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'last_contact':
          ...
          break
        case 'last_visit':
          const visitA = a.last_visit ? new Date(a.last_visit).getTime() : 0
          const visitB = b.last_visit ? new Date(b.last_visit).getTime() : 0
        ...
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className="text-sm text-blue-700">
              ...
              <Link href="/admin/customers" className="font-medium underline hover:text-blue-600">
                Klanten Overzicht
              </Link>.
            </p>
          </div>
                ...
                {getVisibleColumns().map(([columnKey, config]) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span 
                        className={`cursor-move ${columnKey !== 'actions' ? 'cursor-pointer hover:text-blue-600' : ''}`}
                    ...
                    >
                      ...
                      {columnKey === 'last_contact' && (
                        <div className="text-sm text-gray-500">
                          {customer.last_contact ? new Date(customer.last_contact).toLocaleDateString('nl-NL') : '-'}
                        </div>
                      )}
                      {columnKey === 'last_visit' && (
                        <div className="text-sm text-gray-500">
                          {customer.last_visit ? new Date(customer.last_visit).toLocaleDateString('nl-NL') : '-'}
                        </div>
                      )}
                      {columnKey === 'target' && (
          ...
          {isViewing && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-blue-600">{formData.total_orders}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
