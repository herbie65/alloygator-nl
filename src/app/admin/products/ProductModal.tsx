...
export default function ProductModal({ product, isEditing, isOpen, onClose, onSave }: ProductModalProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [attributes, colors] = await Promise.all([
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.description) {
      const productData = {
        ...formData,
        ...dynamicValues, // Include dynamic attribute values
              ...
              <div>
                ...
                {productColors.length === 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    ...
                    <a href="/admin/settings/colors" className="text-blue-600 hover:underline ml-1">
