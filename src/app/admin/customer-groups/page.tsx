...
export default function CustomerGroupsPage(){
  ...
  useEffect(()=>{ (async ()=>{
    ...
    }catch(e:any){ setError(e?.message||'Fout bij laden'); }
    finally{ setLoading(false) }
  })() },[])

  const updateTarget = async (id:string, target:number)=>{
    try{
      await FirebaseClientService.updateDocument('customer_groups', id, { annual_target_sets: target, updated_at: new Date().toISOString() })
